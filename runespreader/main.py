...
class Runespreader:
    def __init__(self):
        ...
        raw_list = requests.get("https://prices.runescape.wiki/api/v1/osrs/mapping", headers=self.custom_headers).json()
    ...
    def get_latest_data_for_id(self, uuid):
        data_dict = requests.get(f"https://prices.runescape.wiki/api/v1/osrs/latest/?id={uuid}", headers=self.custom_headers).json().get("data").get(str(uuid))
        resp_json = requests.get("https://prices.runescape.wiki/api/v1/osrs/volumes", headers=self.custom_headers).json()
    ...
    def get_latest_data_for_all_symbols(self):
        data_dict = requests.get(f"https://prices.runescape.wiki/api/v1/osrs/latest/", headers=self.custom_headers).json().get("data")
        data_array = []
        for entry_key, entry_val in data_dict.items():
            entry_val["name"] = self.get_name_for_id(int(entry_key))
            entry_val["id"] = int(entry_key)
            if not entry_val.get("high") or not entry_val.get("low") or not entry_val.get("name"):
                continue 
